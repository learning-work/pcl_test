...
pcl::PointCloud<pcl::PointXYZ>::Ptr rect_model;
//手动创建一个完整矩形柱
void GenerateCube(double half_r, double res)
    {
        ...
        for(double x = x_min; x <= x_max + 1e-9; x += res){
            for(double y = y_min; y <= y_max + 1e-9; y += res){
                pt.x = x; pt.y = y; pt.z = z_min; rect_model->points.push_back(pt);
            }
        }
        ...
        for(double x = x_min; x <= x_max + 1e-9; x += res){
            for(double y = y_min; y <= y_max + 1e-9; y += res){
                pt.x = x; pt.y = y; pt.z = z_max; rect_model->points.push_back(pt);
            }
        }
        ...
        for(double z = z_min; z <= z_max + 1e-9; z += res){
            for(double y = y_min; y <= y_max + 1e-9; y += res){
                pt.x = x_min; pt.y = y; pt.z = z; rect_model->points.push_back(pt);
                pt.x = x_max; pt.y = y; pt.z = z; rect_model->points.push_back(pt);
            }
        }
        ...
        for(double z = z_min; z <= z_max + 1e-9; z += res){
            for(double x = x_min; x <= x_max + 1e-9; x += res){
                pt.x = x; pt.y = y_min; pt.z = z; rect_model->points.push_back(pt);
                pt.x = x; pt.y = y_max; pt.z = z; rect_model->points.push_back(pt);
            }
        }
    }
...
public:
    ...
    pcl::PointCloud<pcl::FPFHSignature33>::Ptr& features)
    {
        ...
        fpfh.setRadiusSearch(0.15);  // 根据雷达尺度调
    ...
    //
    float computeFPFHSimilarity(
    const pcl::PointCloud<pcl::FPFHSignature33>::Ptr& cluster_feat,
    const pcl::PointCloud<pcl::FPFHSignature33>::Ptr& model_feat)
    {
        pcl::KdTreeFLANN<pcl::FPFHSignature33> tree;
        tree.setInputCloud(model_feat);

        float total_dist = 0.0f;
        int valid_cnt = 0;

        for (const auto& f : cluster_feat->points)
        {
            if (!pcl::isFinite(f))
                continue;
            ...
            if (tree.nearestKSearch(f, 1, idx, dist) > 0)
            {
                total_dist += dist[0];
                valid_cnt++;
            }
        ...
        return total_dist / valid_cnt;
    }


    //判断是不是一个矩形柱
    bool isRectangularLike(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr& cluster,
    const pcl::PointCloud<pcl::PointXYZ>::Ptr& rect_model,
    float& score)
    {
        // cluster
        pcl::PointCloud<pcl::Normal>::Ptr cluster_normals(new pcl::PointCloud<pcl::Normal>);
        pcl::PointCloud<pcl::FPFHSignature33>::Ptr cluster_feat(new pcl::PointCloud<pcl::FPFHSignature33>);

        // model
        pcl::PointCloud<pcl::Normal>::Ptr model_normals(new pcl::PointCloud<pcl::Normal>);
        pcl::PointCloud<pcl::FPFHSignature33>::Ptr model_feat(new pcl::PointCloud<pcl::FPFHSignature33>);

        estimateNormals(cluster, cluster_normals);
        estimateNormals(rect_model, model_normals);

        computeFPFH(cluster, cluster_normals, cluster_feat);
        computeFPFH(rect_model, model_normals, model_feat);

        score = computeFPFHSimilarity(cluster_feat, model_feat);
    ...
    auto cloud_callback = [&](const sensor_msgs::PointCloud2::ConstPtr& msg)
    {
        ...
        for(const auto& indices: cluster_incices)
        {
            ...
            for(int idx: indices.indices)
                ...
                if(pillar.isRectangularLike(cluster, rect_model,souce))
                {
